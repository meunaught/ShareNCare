...
import com.example.sharencare.adapter.Sent_RequestsAdapter
...
class ReceivedRequestsFragment : Fragment() {
    ...
    private var userAdapter : Received_RequestsAdapter? = null
    private var mUser : MutableList<User>?= null
    private var sUser : MutableList<User> = ArrayList()
    private var receivedRequestsList : MutableList<User>?= null
    private lateinit var search_editText_received_requests_fragment : EditText
    ...
    override fun onCreateView(
        ...
        val view =  inflater.inflate(R.layout.fragment_received_requests, container, false)
        ...
        search_editText_received_requests_fragment.addTextChangedListener(object : TextWatcher
        {
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                if(search_editText_received_requests_fragment.text.toString()== "")
                {
                    mUser?.clear()
                    for(user in sUser)
                    {
    ...
    private fun checkRequests() {
        ...
        followingRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists())
                {
                    (receivedRequestsList as ArrayList<String>).clear()
                    for(temp_snapshot in snapshot.children){
                        temp_snapshot.key?.let{(receivedRequestsList as ArrayList<String>).add(it)}
                    }
                    retrieveUsers()
                }
            }
